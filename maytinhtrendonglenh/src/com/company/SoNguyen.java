...
public class SoNguyen {
    ...
    public BigInteger giaiThua (BigInteger operand) {
        BigInteger kq = operand;
        operand = operand.subtract(BigInteger.valueOf(1));
        while (! operand.equals(BigInteger.ZERO)) {
            kq = kq.multiply(operand);
            operand = operand.subtract(BigInteger.valueOf(1));
        }
        return kq;
    }
    ...
    public String valueMath (String[] elementMath) {
        ...
        for (int i = 1; i < elementMath.length; i++) {
            ...
            if (! this.isOperator(c)) {
                if (precedence(c) == - 1) {
                    String s = "Biểu thức chứa ký tự không phải số nguyên hay toán tử!";
                    return s;
                } else ketQua.push(elementMath[i]);
            }
            else {
